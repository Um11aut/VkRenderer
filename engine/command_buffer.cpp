...
VkRenderer::CommandBuffer::CommandBuffer(Extra::VkVars* vars, std::shared_ptr<SwapChain> swapChain, std::shared_ptr<GraphicsPipeline> graphicsPipeline)
	: m_swapChain(swapChain), m_graphicsPipeline(graphicsPipeline)
{
	...
	m_poolInfo.queueFamilyIndex = m_queueFamilyIndices.graphicsFamily.value();
...
void VkRenderer::CommandBuffer::record(uint32_t imageIndex)
{
	...
	vkCmdBindPipeline(m_commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, m_graphicsPipeline->getPipeline());
